...
function ProjectDialog({ detail, close }: { detail: Detail | null; close: () => void }) {
  ...
  return <dialog ref={dialog} className="project-dialog" aria-labelledby="dialog-title" onCancel={close} onClick={event => { if (event.target === event.currentTarget) close(); }}>
    <div className="dialog-inner"><button className="dialog-close icon-button" onClick={close} aria-label="ปิดรายละเอียด"><X /></button><span className="eyebrow">{personal ? detail.type : `${detail.kind.toUpperCase()} · ${workExperience.company}`}</span><h2 id="dialog-title">{detail.title}</h2><span className="role-label">{detail.role}{!personal && ` · ${workExperience.period}`}</span>{personal && hasScreenshot(detail) && <Gallery key={detail.id} project={detail} />}{!personal && <div className="work-summary-box"><span>โปรเจกต์นี้คืออะไร</span><p>{detail.summary}</p></div>}<p className="detail-description">{detail.description}</p>{!personal && <><h3>สิ่งที่รับผิดชอบ</h3><ul className="feature-list">{detail.features.map(feature => <li key={feature}>{feature}</li>)}</ul></>}<h3>Tools & technologies</h3><div className="tags">{detail.technologies.map(tech => <span key={tech}>{tech}</span>)}</div><div className="dialog-links">{detail.demoUrl && <a className="button primary" href={detail.demoUrl} target="_blank" rel="noreferrer">เปิดเว็บไซต์ <ExternalLink size={15} /></a>}{repositories.map(([label, url]) => <a key={label} className="button secondary" href={url} target="_blank" rel="noreferrer"><Github size={15} />{label}</a>)}</div>{!detail.demoUrl && repositories.length === 0 && <p className="private-note"></p>}</div>
  </dialog>;
}
...
export default function App() {
  ...
  // Its JSX seed stays constant so changing the chapter label cannot overwrite the live altitude.
  ...
  return <div className={`app ${paused ? 'motion-paused' : ''}`} data-chapter={active}>
    ...
    <main id="main">
      ...
      <section id="sky" className="sky-section chapter section-padding"><div className="chapter-intro"><div className="chapter-heading"><span className="eyebrow">03 / ท้องฟ้า · พื้นที่ทดลอง</span><span className="altitude">ต่อยอดสิ่งที่ได้เรียนรู้</span></div><h2>ลองสิ่งใหม่<br /><span className="chapter-subtitle">ให้ไอเดียได้ออกบิน</span></h2><p className="section-description">เมื่อมีพื้นฐานที่มั่นคง ผมก็อยากลองไปให้ไกลขึ้น โปรเจกต์เหล่านี้คือพื้นที่ทดลองของผม ตั้งแต่เว็บและแอป ไปจนถึง AI เพื่อเปลี่ยนสิ่งที่สงสัยให้เป็นผลงานที่เปิดใช้งานได้</p></div><div className="chapter-content">
